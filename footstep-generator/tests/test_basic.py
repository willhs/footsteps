...
from models import (
    ...
    LODConfiguration, ProcessingResult
)
...
def test_basic_functionality():
    ...
    print(f"    Created settlement: {settlement.population} people at ({coords.longitude}, {coords.latitude})")
    ...
    print(f"    Created processor with global grid size: {processor.config.global_grid_size}°")
    ...
    for level, lod_settlements in lod_data.items():
        if lod_settlements:
            ...
            print(f"      {level.name}: {len(lod_settlements)} settlements (pop ratio: {ratio:.3f})")
    ...
    city_dots = processor.create_density_aware_dots(50000.0, 45.0, -100.0, 0.083333, 100)
    ...
    for zoom, expected_lod in zoom_tests:
        ...
        assert actual_lod == expected_lod, f"Zoom {zoom} should map to {expected_lod}, got {actual_lod}"
...
def test_error_handling():
    ...
    # Test empty data
    empty_result = processor.create_hierarchical_lods([])
    assert all(len(settlements) == 0 for settlements in empty_result.values()), "Empty input should return empty LOD data"
    print("  ✓ Empty data handling works")
    
    # Test invalid coordinates (will be caught by Pydantic during model creation)
    try:
        invalid_coords = Coordinates(longitude=200.0, latitude=0.0)  # Invalid longitude
    ...
    try:
        coords = Coordinates(longitude=0.0, latitude=0.0)
        invalid_settlement = HumanSettlement(
            coordinates=coords,
            population=-100.0,  # Negative population
            year=1000,
            source_resolution=0.083333
        )
        assert False, "Should have raised ValueError for negative population"
    except ValueError:
        print("  ✓ Invalid population validation works")
