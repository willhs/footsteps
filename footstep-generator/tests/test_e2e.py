...
from models import (
    ...
    LODConfiguration, ProcessingStatistics, HYDEDataFile, GridMetadata
)
...
class TestDataProcessingPipeline:
    ...
    def create_test_settlements(self, count: int = 100, year: int = 0) -> List[HumanSettlement]:
    ...
    def test_population_conservation(self):
        ...
        for lod_level, aggregated_settlements in lod_data.items():
            lod_total_population = sum(s.total_population for s in aggregated_settlements)
            
            # Allow for small rounding errors and filtering
            population_ratio = lod_total_population / original_population
            assert 0.9 <= population_ratio <= 1.1, f"Population not conserved at {lod_level}: {population_ratio:.3f}"
    ...
    def test_density_aware_dot_creation(self):
        ...
        assert len(rural_dots) <= 5  # Should create reasonable number of dots
        ...
        assert len(town_dots) <= 5  # Should limit dots to avoid clutter
        ...
        assert len(city_dots) <= 3  # Should create few large dots for cities
